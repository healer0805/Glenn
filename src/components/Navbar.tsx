...
const Navbar = () => {
  ...
  const menuItems = [
    ...
    { path: '/services', label: 'FULL STACK DEVELOPMENT' },
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 py-3 md:py-4">
        <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div style={{ mixBlendMode: 'difference' }}>
            <button
              onClick={toggleMenu}
              className="flex flex-col gap-[5px] bg-transparent border-none cursor-pointer p-2 outline-none focus:outline-none"
              aria-label="Toggle menu"
              onFocus={(e) => e.target.blur()}
            >
              <div className="w-8 md:w-14 h-[1px] bg-white transition-all duration-300"></div>
              <div className="w-8 md:w-14 h-[1px] bg-white transition-all duration-300"></div>
              <div className="w-8 md:w-14 h-[1px] bg-white transition-all duration-300"></div>
              <div className="w-8 md:w-14 h-[1px] bg-white transition-all duration-300"></div>
            </button>
      ...
      >
        <div className="w-full h-full bg-[#151513] flex flex-col md:flex-row relative overflow-hidden">
          ...
          <div className="flex-1 flex flex-col justify-between p-6 md:p-12 relative overflow-y-auto">
            ...
            <button
              onClick={closeMenu}
              className="absolute top-4 right-4 md:top-8 md:right-8 text-white text-4xl font-light bg-transparent border-none cursor-pointer hover:opacity-80 transition-opacity z-10 outline-none focus:outline-none"
                ...
                {menuItems.map((item) => (
                  <li key={item.path} className="m-0">
                    <button
                      ...
                      className="text-white bg-transparent border-none cursor-pointer text-left hover:opacity-80 transition-opacity outline-none focus:outline-none break-words"
                      style={{
                        fontFamily: '"Anton", "Anton Placeholder", sans-serif',
                        fontSize: item.label === 'FULL STACK DEVELOPMENT' 
                          ? 'clamp(24px, 4vw, 50px)' 
                          : 'clamp(32px, 5vw, 60px)',
                        fontStyle: 'normal',
                        fontWeight: 400,
